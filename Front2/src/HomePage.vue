...
const socket2 = io(import.meta.env.VITE_SOCKET_URL2,{
  withCredentials:true
})
...
async function likes(post) {
  const id = post.id;
  const res = await axios.post(`/api/blog/post/${id}/like`,{},{withCredentials:true});
  post.likes = res.data.post.likes;
}

async function dislikes(post) {
  const id = post.id;
  const res = await axios.post(`/api/blog/post/${id}/dislike`,{},{withCredentials:true});
